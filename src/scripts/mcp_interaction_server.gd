...
func _cmd_ui_theme(params: Dictionary) -> void:
	...
	for constant_name in constants:
		ctrl.add_theme_constant_override(constant_name, int(constants[constant_name]))
		applied.append("constant:" + constant_name)

	# Font size overrides
	var font_sizes: Dictionary = overrides.get("fontSizes", overrides.get("font_sizes", {}))
	for font_size_name in font_sizes:
		ctrl.add_theme_font_size_override(font_size_name, int(font_sizes[font_size_name]))
		applied.append("font_size:" + font_size_name)
...
func _cmd_input_state(params: Dictionary) -> void:
	...
	match action:
		...
		"set_mouse_mode":
			...
			match mode_str:
				"hidden": Input.mouse_mode = Input.MOUSE_MODE_HIDDEN
				"captured": Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
				"confined": Input.mouse_mode = Input.MOUSE_MODE_CONFINED
...
func _cmd_process_mode(params: Dictionary) -> void:
	...
	match mode_str:
		"pausable": node.process_mode = Node.PROCESS_MODE_PAUSABLE
		"when_paused": node.process_mode = Node.PROCESS_MODE_WHEN_PAUSED
		"always": node.process_mode = Node.PROCESS_MODE_ALWAYS
		"disabled": node.process_mode = Node.PROCESS_MODE_DISABLED
		_: node.process_mode = Node.PROCESS_MODE_INHERIT
